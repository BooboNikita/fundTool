...
export async function getFundInfoMap(): Promise<Map<string, FundInfo>> {
  const now = Date.now();
...
export async function getPortfolio(
  ...
  try {
    let query =
      "SELECT id, code, name, note, is_watchlist, is_holding, created_at FROM funds WHERE user_id = ?";
...
export async function getPortfolioEstimation(
  ...
  try {
    let query =
      "SELECT id, code, name, note, is_watchlist, is_holding FROM funds WHERE user_id = ?";
    const params: any[] = [userId];
...
export async function getTop10FundsData(): Promise<any[]> {
  ...
  return validEstimations.slice(0, 10);
}
