...
async function sendDingTalkMessage(
  ...
  try {
    ...
    if (error.response) {
      console.error("DingTalk response status:", error.response.status);
      console.error("DingTalk response data:", error.response.data);
    }
    return false;
  }
}
...
function formatTableMessage(
  estimations: any[],
  categories: { watchlist: any[]; holding: any[] },
): string {
...
async function doPushForUser(config: DingTalkConfigFull): Promise<boolean> {
  try {
    let query =
      "SELECT code, name, is_watchlist, is_holding FROM funds WHERE user_id = ? AND (";
    ...
    const categories = {
      watchlist: config.push_watchlist
        ? estimations.filter((e) => e.is_watchlist)
        : [],
      holding: config.push_holding
        ? estimations.filter((e) => e.is_holding)
        : [],
    };
...
export async function getDingTalkConfig(
  ...
  try {
    const [rows] = await db.execute(
      "SELECT id, user_id, webhook_url, secret, push_times, push_interval_hours, push_enabled, push_watchlist, push_holding, last_push_time FROM dingtalk_config WHERE user_id = ?",
      [userId],
    );
...
export async function testDingTalkPush(
  req: AuthRequest,
  res: Response,
): Promise<void> {
  const userId = req.userId;
...
export async function pushFundEstimations(
  req: AuthRequest,
  res: Response,
): Promise<void> {
  const userId = req.userId;
...
export async function startScheduler(): Promise<void> {
  ...
  const checkAndPush = async () => {
    try {
      const [configRows] = await db.execute(
        "SELECT id, user_id, webhook_url, secret, push_times, push_interval_hours, push_enabled, push_watchlist, push_holding, last_push_time FROM dingtalk_config WHERE push_enabled = TRUE",
      );
